...
시스템의 안정성을 위해 다음 규칙을 엄격히 준수하여 개발되었습니다.

* 프롬프트 템플릿 보호: ChatPromptTemplate 구성 시 파이썬 f-string을 절대 사용하지 않습니다. 크롤링된 웹페이지 본문 텍스트 내에 포함된 중괄호({})나 특수 자바스크립트 기호가 템플릿 변수와 충돌하여 파싱 오류가 발생하는 것을 차단하기 위해, 텍스트 데이터는 체인 실행 시(.invoke) 딕셔너리 형태로 안전하게 주입합니다.
* 추론 파라미터 최적화: 온도(temperature) 조절 대신 최신 모델의 reasoning_effort 파라미터를 사용하여, 창의적인 답변보다는 빠르고 논리적인 사실 요약에 집중하도록 설계했습니다.
